...
const is_konqueror = (agent.indexOf('konqueror') != -1);
const is_safari = (agent.indexOf('safari') != -1) || is_konqueror;
const is_nav = !is_ie && !is_safari && (agent.indexOf('mozilla') != -1);
const is_win = (agent.indexOf('win') != -1);
delete agent;


const BACKSPACE_KEYCODE = 8;
const COMMA_KEYCODE = 188;                // ',' key
const DEBUG_KEYCODE = 68;                 // 'D' key
const DELETE_KEYCODE = 46;
const DOWN_KEYCODE = 40;                  // DOWN arrow key
const ENTER_KEYCODE = 13;                 // ENTER key
const ESC_KEYCODE = 27;                   // ESC key
const LEFT_KEYCODE = 37;                  // LEFT arrow key
const RIGHT_KEYCODE = 39;                 // RIGHT arrow key
const SPACE_KEYCODE = 32;                 // space bar
const TAB_KEYCODE = 9;                    // TAB key
const UP_KEYCODE = 38;                    // UP arrow key
...
function AssertType(value, type, opt_msg) {
  ...
  if (opt_msg === undefined) {
    if (typeof type == 'function') {
      const match = type.toString().match(/^\s*function\s+([^\s\{]+)/);
...
// Cookies
//------------------------------------------------------------------------
const ILLEGAL_COOKIE_CHARS_RE = /[\s;]/;
/**
 * Sets a cookie.
...
function SetCookie(name, value, opt_max_age, opt_path, opt_domain) {
  ...
  const domain_str = (opt_domain == null) ? '' : ';domain=' + opt_domain;
  const path_str = (opt_path == null) ? '' : ';path=' + opt_path;
  ...
  // Case 3: Set a persistent cookie.
    ...
    const futureDate = new Date(Now() + opt_max_age * 1000);
...
function GetCookie(name) {
  ...
  for (let pos = -1; (pos = cookie.indexOf(nameeq, pos + 1)) >= 0;) {
    let i = pos;
...
function GetPageOffset(el) {
  ...
  let y = el.offsetTop;
  if (el.offsetParent != null) {
    const pos = GetPageOffset(el.offsetParent);
    x += pos.x;
    y += pos.y;
  }
  return {x: x, y: y};
...
function SetCursorPos(win, textfield, pos) {
  if (IsDefined(textfield.selectionEnd) &&
    ...
    const sel = textfield.createTextRange();
 ...
 */
const forid = document.getElementById ? forid_1 : forid_2;
...
function log(msg) {
  ...
  const logPane = forid('log');
  if (logPane) {
    const logText = '<p class=logentry><span class=logdate>' + new Date() +
